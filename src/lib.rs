...
mod culidex {
    ...
    fn cosine_scores(matrix : Vec<Vec<f64>>, query : Vec<f64>, top_n : usize) -> Vec<(usize, f64)> {
        let mut scores : Vec<(usize, f64)> = matrix.iter().enumerate().map(|(i, row)| {
            ...
            let row_norm : f64 = row.iter().zip(&query).map(|(a, _)| a * a).sum::<f64>().sqrt();

            let query_norm : f64 = row.iter().zip(&query).map(|(_, b)| b * b).sum::<f64>().sqrt();
    ...
    //#[pyfunction]
    //fn cosine_scores2(matrix: Vec<Vec<f64>>, query: Vec<f64>, top_n: usize) -> Vec<(usize, f64)> {
